  ...
  <div class="goods">
    <div class="scroll-nav-wrapper">
      <cube-scroll-nav :side="true" :data="goods" :options="scrollOptions" v-if="goods.length">
        <template v-slot:bar="{ labels, current }">
          <cube-scroll-nav-bar direction="vertical" :labels="labels" :txts="barTxts()" :current="current">
...
export default {
  ...
  methods: {
     ...
     */
    async fetch() {
      // this.fetched 的作用是为了在滑动到其他tab时,再滑回来,不再次获取goods数据
      if (!this.fetched) {
        this.fetched = true
